...
RUN --mount=type=cache,id=s/6bdb5fc7-8c9c-43ee-a9c7-f47d441aece1-apt-cache,target=/var/cache/apt,sharing=locked \
  --mount=type=cache,id=s/6bdb5fc7-8c9c-43ee-a9c7-f47d441aece1-apt-lists,target=/var/lib/apt/lists,sharing=locked \
  apt-get update \
  && apt-get upgrade -y \
...
# broken image.
RUN --mount=type=cache,id=s/6bdb5fc7-8c9c-43ee-a9c7-f47d441aece1-npm-cache,target=/root/.npm \
...
COPY . ./
RUN --mount=type=cache,id=s/6bdb5fc7-8c9c-43ee-a9c7-f47d441aece1-next-cache,target=/app/.build/next/cache \
  mkdir -p /app/data \
  && npm run build \
